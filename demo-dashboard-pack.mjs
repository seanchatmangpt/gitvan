...
class DashboardPackDemo {
  ...
  async run() {
    console.log("🚀 Starting Hyper-Advanced Dashboard Pack Demo");
  ...
  async createProject() {
    ...
    const packTemplatesPath = join(process.cwd(), "packs", "nextjs-dashboard-pack", "templates");
...
@layer base {
  * {
  ...
  async startDevServer() {
    ...
    console.log(`   cd ${this.projectName} && pnpm dev`);
    console.log("📊 Dashboard will be available at: http://localhost:3000/dashboard");
  }
...
// Run the demo
const demo = new DashboardPackDemo();
