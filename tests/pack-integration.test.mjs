...
`);
    ...
    writeFileSync(jobFile, `// Build job template
export default {
  name: 'build',
  ...
  async run(context) {
    ...
    const generatedConfig = JSON.parse(readFileSync(join(outputDir, 'package.json'), 'utf8'));
    ...
    const copiedScript = readFileSync(join(outputDir, 'scripts', 'build.mjs'), 'utf8');
  ...
  it('should handle error scenarios gracefully', async () => {
    ...
    const templateProcessor = new TemplateProcessor();
    await expect(templateProcessor.process({
    ...
    const transformProcessor = new TransformProcessor();
    await expect(transformProcessor.apply({
    ...
    const jobInstaller = new JobInstaller();
    await expect(jobInstaller.install({
