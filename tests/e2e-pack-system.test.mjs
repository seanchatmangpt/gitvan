...
import { PromptingContext, promptForInputs, createPromptFromInput } from '../src/utils/prompts.mjs';
import {
  satisfiesConstraint,
  compareVersions,
  isValidVersion,
...
describe('GitVan Pack System - End-to-End Verification', () => {
  beforeAll(async () => {
    ...
    tempTestDir = join(tmpdir(), 'gitvan-e2e-test-' + randomBytes(8).toString('hex'));
  ...
  describe('🔍 Registry System - Real Pack Discovery', () => {
    ...
    it('PROOF: Registry actually scans filesystem and finds real packs', async () => {
      ...
      console.log('✅ VERIFIED: Registry scans filesystem and finds real packs');
    }, TEST_TIMEOUT);

    it('PROOF: Builtin packs actually exist and load', async () => {
      ...
      const expectedBuiltins = ['next-minimal', 'nodejs-basic', 'docs-enterprise'];
  ...
  describe('🔐 Security System - Real Cryptographic Operations', () => {
    ...
    beforeEach(async () => {
      ...
      const { publicKey, privateKey } = await signer.generateKeyPair({ keySize: 2048 });
      ...
      writeFileSync(join(testPackDir, 'pack.json'), JSON.stringify(manifest, null, 2));
    });

    it('PROOF: Security system uses real RSA crypto (not mock-sha256)', async () => {
      ...
      const tamperedManifest = { id: 'tampered', name: 'Tampered', version: '999.0.0' };
      writeFileSync(join(testPackDir, 'pack.json'), JSON.stringify(tamperedManifest, null, 2));

      const tamperedVerification = await signer.verify(testPackDir, publicKeyPath);
  ...
  describe('🔍 Search System - Real Fuzzy Search', () => {
    ...
    it('PROOF: Search returns real fuzzy matching results', async () => {
      ...
      const reactPack = reactResults.find(p => p.name === 'React Frontend Starter');
      ...
      const advancedResults = await registry.advancedSearch('tag:vue capability:scaffold');
      expect(advancedResults.results.length).toBeGreaterThanOrEqual(1);
      const vuePack = advancedResults.results.find(p => p.name === 'Vue Dashboard');
      ...
      console.log('✅ VERIFIED: Search uses real fuzzy matching with Fuse.js');
    }, TEST_TIMEOUT);
  });
  ...
  describe('💾 Cache System - Real Disk Persistence', () => {
    ...
    beforeEach(async () => {
      cacheTestDir = join(testCacheDir, 'cache-test-' + randomBytes(4).toString('hex'));
      cache = new PackCache({
    ...
    it('PROOF: Cache actually persists to real disk storage', async () => {
      const testData = {
        id: 'test-pack',
        name: 'Test Pack',
        version: '1.0.0',
      ...
      const persistedData = await newCache.get('pack-info', { packId: 'test' });
      ...
      console.log('✅ VERIFIED: Cache persists to real disk storage with compression');
    }, TEST_TIMEOUT);
  });
  ...
  describe('📝 File Operations - Real Glob Patterns', () => {
    ...
    it('PROOF: File operations use real glob patterns with tinyglobby', async () => {
      ...
      const readmeFiles = await fileOps.resolveSources(join(sourceDir, 'README.*'));
  ...
  describe('🎛️ User Prompting - Real Prompt Interactions', () => {
    it('PROOF: Prompting system handles real user interactions', async () => {
      ...
      expect(nonInteractiveContext.getPreAnswer('projectName')).toBe('default-project');

      // VERIFY: Different input types create correct prompts
      const booleanInput = { key: 'useTypeScript', type: 'boolean', description: 'Use TypeScript?' };
      ...
      console.log('✅ VERIFIED: Prompting system handles real user interactions');
    });
  });
  ...
  describe('🎨 Template Processor - Real Format Support', () => {
    ...
    it('PROOF: Template processor handles multiple formats with real Nunjucks', async () => {
...
`;
      ...
      const filterResult = await processor.render(filterTemplate, filterContext);
      ...
      expect(readFileSync(outputPath, 'utf8')).toBe('Hello World! Version: 2.0.0');
      ...
      console.log('✅ VERIFIED: Template processor handles real formats with Nunjucks');
    }, TEST_TIMEOUT);
  });
  ...
  describe('🌐 GitHub Integration - Real API Calls', () => {
    ...
    it('PROOF: GitHub integration makes real API calls', async () => {
      ...
      const parsedWithRef = registry.parseGitHubPackId('ruvnet/claude-flow#v1.0.0');
      expect(parsedWithRef.ref).toBe('v1.0.0');

      const parsedWithPath = registry.parseGitHubPackId('ruvnet/claude-flow/packages/core');
      expect(parsedWithPath.path).toBe('packages/core');

      // VERIFY: Real GitHub API metadata fetch
      const metadata = await registry.fetchGitHubRepoMetadata('octocat', 'Hello-World');
      if (metadata) {
        expect(metadata.full_name).toBe('octocat/Hello-World');
        expect(metadata.stargazers_count).toBeGreaterThanOrEqual(0);
        expect(metadata.clone_url).toBe('https://github.com/octocat/Hello-World.git');
      }
    ...
    it('PROOF: GitHub caching and error handling work', async () => {
      ...
      const invalidMetadata = await registry.fetchGitHubRepoMetadata('nonexistent', 'invalid-repo-' + randomBytes(8).toString('hex'));
      expect(invalidMetadata).toBeNull();

      // VERIFY: Cache key generation
      const cacheKey = registry.generateGitHubCacheKey('owner', 'repo', 'main', 'path/to/pack');
      expect(cacheKey).toBe('github-owner-repo-path-to-pack');

      const cacheKeyWithRef = registry.generateGitHubCacheKey('owner', 'repo', 'v1.0.0', null);
  ...
  describe('🔍 Core Implementation Verification', () => {
    it('PROOF: Core pack implementations are real (not placeholders)', async () => {
      ...
      const result = await processor.render('Hello {{ name }}!', { name: 'World' });
      expect(result).toBe('Hello World!');

      console.log('✅ VERIFIED: All core implementations are real and functional');
    });
  });

  describe('📊 Integration Test - Full Pack Lifecycle', () => {
    it('PROOF: Complete pack system works end-to-end', async () => {
      ...
      writeFileSync(join(customPackDir, 'pack.json'), JSON.stringify(manifest, null, 2));
      ...
      const integrationPack = searchResults.find(p => p.name === 'Integration Test Pack');
      ...
      const cachedData = await registry.packCache.get('integration-test', cacheKey);
...
`;
      ...
      const processedTemplate = await templateProcessor.render(templateContent, manifest);
      ...
      const globResults = await fileOps.resolveSources(join(customPackDir, '*'));
