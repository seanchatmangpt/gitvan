...
class ReleaseAutomation {
  ...
  async publishToNpm() {
    ...
    const publishedVersion = await this.runCommand(`npm view ${PACKAGE_NAME}@${VERSION} version`);
    
    if (publishedVersion !== VERSION) {
      throw new Error(`Version mismatch: expected ${VERSION}, got ${publishedVersion}`);
    }

    this.log(`✅ Successfully published ${PACKAGE_NAME}@${VERSION} to npm`, 'success');
  }
  ...
  async createGitHubRelease() {
    ...
    try {
      ...
      this.log('Please create the release manually at: https://github.com/seanchatmangpt/gitvan/releases/new');
    }
  }
  ...
  async postReleaseValidation() {
    ...
    try {
      ...
      await this.runCommand(`cd ${testDir} && npm install ${PACKAGE_NAME}@${VERSION}`);
      const version = await this.runCommand(`cd ${testDir} && npx gitvan --version`);
  ...
  async run() {
    try {
      ...
      this.log(`  - GitHub: https://github.com/seanchatmangpt/gitvan/releases/tag/v${VERSION}`);
      this.log('');
      this.log('🎯 Next Steps:');
