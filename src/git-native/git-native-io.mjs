...
export class GitNativeIO {
  ...
  async initialize() {
    ...
    const { QueueManager } = await import("./queue-manager.mjs");
    const { LockManager } = await import("./lock-manager.mjs");
    const { ReceiptWriter } = await import("./receipt-writer.mjs");
    const { SnapshotStore } = await import("./snapshot-store.mjs");
    const { WorkerPool } = await import("./worker-pool.mjs");
    ...
    // Set initialized flag before reconciliation
  ...
  async reconcile() {
    // Don't call _ensureInitialized() here as reconcile() is called during initialization
    if (!this._initialized) {
      throw new Error("GitNativeIO must be initialized before reconciliation");
    }
