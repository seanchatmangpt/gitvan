...
import { PackSigner, ReceiptManager, PolicyEnforcer, createSecurityContext, validateSecurityConfiguration } from '../../../src/pack/security/index.mjs';
...
describe('Security Integration', () => {
  ...
  beforeEach(async () => {
    ...
    securityContext = createSecurityContext({
      ...
      receipts: { receiptsRef: 'refs/notes/gitvan/test-receipts', gitvanVersion: '2.0.0-test' },
  ...
  describe('validateSecurityConfiguration', () => {
    it('should validate correct security configuration', () => {
      ...
      const result = validateSecurityConfiguration(config);
    ...
    it('should reject invalid receipt ref', () => {
      const config = {
      ...
      const result = validateSecurityConfiguration(config);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Receipt ref must be in refs/notes/ namespace');
    });
  });
  ...
  describe('Full Security Workflow', () => {
    it('should complete secure pack processing workflow', async () => {
      ...
      const policyResult = await securityContext.policy.enforce(pack, 'install');
      expect(policyResult.allowed).toBe(true);

      // Step 2: Create installation receipt
      const installReceipt = await securityContext.receipts.create(pack, 'install', 'started');
      ...
      const successReceipt = await securityContext.receipts.create(pack, 'install', 'success', {
        duration: 1200,
        filesCreated: 3
      });
    ...
    it('should handle signed pack workflow', async () => {
      ...
      const signedContext = createSecurityContext({
        signer: { signer: 'test-authority' },
        receipts: { receiptsRef: 'refs/notes/gitvan/signed-receipts', gitvanVersion: '2.0.0-test' },
      ...
      const signatureData = await signedContext.signer.sign(packPath, join(testDir, 'private.pem'));
      expect(signatureData.pack_id).toBe('secure-test-pack');
      expect(signatureData.signer).toBe('test-authority');

      // Step 2: Verify signature
      const verificationResult = await signedContext.signer.verify(packPath, join(testDir, 'public.pem'));
      ...
      const receipt = await signedContext.receipts.create(pack, 'install', 'success');
    ...
    it('should reject unsigned pack when signature required', async () => {
      ...
      expect(policyResult.violations).toContain('Pack signature required but not found');
    });
    ...
    it('should handle malicious pack detection', async () => {
      ...
      expect(policyResult.violations).toContain('System command not allowed: rm -rf /');
      expect(policyResult.violations).toContain('Hook command not allowed in pre-install: sudo chmod 777 /');
      expect(policyResult.violations).toContain('Restricted environment variable: PATH');
    });
    ...
    it('should handle receipt verification failure', async () => {
      ...
      // Create receipt
      const receipt = await securityContext.receipts.create(pack, 'install', 'success');
    ...
    it('should export security audit trail', async () => {
      ...
      for (const operation of ['install', 'update', 'verify']) {
        const receipt = await securityContext.receipts.create(pack, operation, 'success');
  ...
  describe('Error Handling', () => {
    it('should handle missing pack manifests gracefully', async () => {
      ...
      await expect(securityContext.signer.sign(emptyPackPath, 'nonexistent-key'))
    ...
    it('should handle invalid signature files', async () => {
      ...
      const result = securityContext.signer.validateSignatureFormat(signaturePath);
      expect(result.valid).toBe(false);
    ...
    it('should handle missing Git repository for receipts', async () => {
      ...
      const receipt = await securityContext.receipts.create(pack, 'test', 'success');
  ...
  describe('Performance and Scalability', () => {
    it('should handle large number of receipts efficiently', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const receiptPromise = securityContext.receipts.create(pack, operation, 'success', { iteration: i })
      ...
      const allReceipts = await securityContext.receipts.read('secure-test-pack');
    ...
    it('should validate policy configuration efficiently', async () => {
      const complexPolicy = new PolicyEnforcer({
        policies: {
          allowedCapabilities: new Array(100).fill(0).map((_, i) => `capability-${i}`),
          blockedCapabilities: new Array(50).fill(0).map((_, i) => `blocked-${i}`),
          allowedCommands: new Array(200).fill(0).map((_, i) => `command-${i}`),
          blockedCommands: new Array(100).fill(0).map((_, i) => `blocked-command-${i}`)
        }
      });
