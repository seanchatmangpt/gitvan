  ...
  for (let i = 0; i < names.length; i += 1) {
    for (let j = i + 1; j < names.length; j += 1) {
      for (const left of parameters[names[i]]) {
        for (const right of parameters[names[j]]) {
          result.add(`${names[i]}=${JSON.stringify(left)}|${names[j]}=${JSON.stringify(right)}`);
        }
      }
    }
  }
...
function scenarioPairs(scenario, names) {
  ...
  for (let i = 0; i < names.length; i += 1) {
    for (let j = i + 1; j < names.length; j += 1) {
      result.add(`${names[i]}=${JSON.stringify(scenario[names[i]])}|${names[j]}=${JSON.stringify(scenario[names[j]])}`);
    }
  }
  return result;
}
...
export function generatePairwiseScenarios(parametersInput = {}) {
  ...
  const enumerate = (index, current) => {
    if (index === names.length) {
      candidates.push(Object.freeze({ ...current }));
      return;
    }
  ...
  while (uncovered.size) {
    ...
    for (const candidate of candidates) {
      ...
      if (coverage > bestCoverage || (coverage === bestCoverage && canonical(candidate) < canonical(best))) {
        best = candidate;
...
function defaultOutcome(scenario) {
  ...
  if (scenario.subject === "mismatch") return { ok: true, standing: "ALIVE", classification: "VERIFIER_ALIVE" };
...
export class ChicagoCapabilityValidator {
  ...
  scenarios(options = {}) {
    return generatePairwiseScenarios({
  ...
  async validateCapability(capabilityId, scenarios = this.scenarios()) {
    ...
    for (const scenario of scenarios) {
      ...
      const stateAssertions = Object.freeze({
        ...
        failureDoesNotCrown: scenario.failure === "none" || receipt.standing !== "ALIVE",
        noAmbientActuation: receipt.actuates !== true,
      });
      executions.push(Object.freeze({ capability: capability.id, scenario, receipt, stateAssertions }));
    }
  ...
  async validateAll(options = {}) {
    ...
    const receipts = executions.map(item => item.receipt);
    const ledger = new CapabilityClaimLedger(this.registry.list(), { expectedSubject: this.subject });
    for (const receipt of receipts.filter(item => item.subject.sha === this.subject.sha)) ledger.ingest(receipt);
    const claims = ledger.summary();
    const diagnosis = diagnoseCapabilities({ capabilities: this.registry.list(), receipts, expectedSubject: this.subject });
    ...
    const body = Object.freeze({
      ...
      diagnosis,
      repairs,
      complete: executions.length === this.registry.list().length * scenarios.length,
