// tests/jtbd-file-operations-only.test.mjs
// Integration tests that validate meaningful work outcomes using only file operations
...
describe("JTBD File Operations Only Meaningful Work Validation", () => {
    ...
    it("should generate a complete executive dashboard with actionable insights", async () => {
      await withMemFSTestEnvironment(
        ...
        async (env) => {
          ...
          // Step 2: Generate executive dashboard using file write with template content
          const dashboardStep = {
            ...
            config: {
              ...
              operation: "write",
              content: `# Q1 2024 Executive Dashboard

## Key Performance Indicators

Revenue: $${totalRevenue}
Target: $${targets.revenue}
Achievement: ${revenueAchievement.toFixed(1)}%

Units: ${totalUnits}
Target: ${targets.units}
Achievement: ${unitsAchievement.toFixed(1)}%

## Executive Summary

Total Revenue: $${totalRevenue}
Total Units: ${totalUnits}
Revenue Achievement: ${revenueAchievement.toFixed(1)}%
Units Achievement: ${unitsAchievement.toFixed(1)}%

${revenueAchievement >= 100 ? '✅ Revenue target achieved' : '❌ Revenue target missed'}
${unitsAchievement >= 100 ? '✅ Units target achieved' : '❌ Units target missed'}
              ...
              content: JSON.stringify({
                ...
                kpis: {
                  revenue: {
                    ...
                    status: revenueAchievement >= 100 ? "achieved" : "missed"
                  },
                  units: {
                    actual: totalUnits,
                    target: targets.units,
                    achievement: unitsAchievement,
                    status: unitsAchievement >= 100 ? "achieved" : "missed"
                  }
                }
              }, null, 2)
            }
    ...
    it("should generate production-ready TypeScript code from API specifications", async () => {
      await withMemFSTestEnvironment(
        ...
        async (env) => {
          ...
          // Step 2: Generate TypeScript interfaces using file write
          const typesStep = {
            id: "generate-types",
            type: "file",
            config: {
              ...
              operation: "write",
              content: `// Generated TypeScript interfaces for ${apiSpec.service} v${apiSpec.version}
...
export interface ApiusersRequest {
  // No parameters for GET /api/users
}
...
export interface ApiusersIdRequest {
  id: string;
}

export interface ApiusersIdResponse {
  data: User;
}`
            }
          };
