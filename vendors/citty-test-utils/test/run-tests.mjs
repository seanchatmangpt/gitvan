...
async function main() {
  ...
  results.integration = runCommand('pnpm test:integration', 'Running Integration Tests')

  // Run BDD tests
  results.bdd = runCommand('pnpm test:bdd', 'Running BDD Tests')

  // Run coverage analysis
  results.coverage = runCommand('pnpm test:coverage', 'Running Coverage Analysis')
  ...
  if (allPassed) {
    ...
    log(`  ${colors.yellow}pnpm test${colors.reset}           - Run all tests in watch mode`)
    log(`  ${colors.yellow}pnpm test:run${colors.reset}      - Run all tests once`)
    log(`  ${colors.yellow}pnpm test:coverage${colors.reset} - Run tests with coverage`)
    log(`  ${colors.yellow}pnpm test:unit${colors.reset}     - Run unit tests only`)
    log(`  ${colors.yellow}pnpm test:integration${colors.reset} - Run integration tests only`)
    log(`  ${colors.yellow}pnpm test:bdd${colors.reset}     - Run BDD tests only`)
    log(`  ${colors.yellow}pnpm test:watch${colors.reset}    - Run tests in watch mode`)
    log(`  ${colors.yellow}pnpm test:ui${colors.reset}       - Run tests with UI`)
    ...
    log(`  ${colors.yellow}coverage/lcov-report/index.html${colors.reset} - HTML coverage report`)
    log(`  ${colors.yellow}coverage/coverage-final.json${colors.reset} - JSON coverage data`)
