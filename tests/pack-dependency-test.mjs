...
async function testDependencyResolver() {
  ...
  try {
    ...
    console.log('Resolution order:', resolution.map(r => r.id));
    ...
    const conflictRes = await resolver.checkCompatibility('ui-pack', 'conflicting-pack');
...
async function testPackComposer() {
  ...
  try {
    ...
    const validation = await composer.validateComposition(['ui-pack', 'auth-pack']);
    ...
    const conflictValidation = await composer.validateComposition(['ui-pack', 'conflicting-pack']);
