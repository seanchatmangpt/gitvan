...
export default defineJob({
  ...
  async run(context) {
    ...
    try {
      ...
      // Log results
      console.log(
        `🔒 Security Policy Enforcer (${hookName}): ${securityValidation.overallStatus}`
      );
      ...
      console.error(
        `❌ Security Policy Enforcer Error (${hookName}):`,
        error.message
      );
      return {
        success: false,
  ...
  async validateSecurityPolicies(gitState) {
    ...
    const overallStatus = Object.values(policies).every(
      (p) => p.status === "PASS"
    )
      ? "PASS"
      : "FAIL";
    ...
    try {
      ...
      return {
        ...
        message:
          violations.length === 0
            ? "No secrets detected"
            : `${violations.length} potential secrets found`,
      };
    } catch (error) {
      return {
        status: "ERROR",
  ...
  async checkDependencySecurity(gitState) {
    ...
    try {
      ...
      const packageFiles = gitState.stagedFiles.filter(
        (f) =>
      ...
      return {
        ...
        message:
          violations.length === 0
            ...
            : `${violations.length} security issues found`,
      };
    } catch (error) {
      return {
        status: "ERROR",
  ...
  async validateAccessControl(gitState) {
    ...
    const accessControlFiles = gitState.stagedFiles.filter(
      (f) =>
        f.includes("auth") ||
  ...
  async checkGDPRCompliance(gitState) {
    ...
    const gdprFiles = gitState.stagedFiles.filter(
      (f) =>
  ...
  async checkSOXCompliance(gitState) {
    ...
    const soxFiles = gitState.stagedFiles.filter(
      (f) => f.includes("audit") || f.includes("financial") || f.includes("sox")
    );
  ...
  async checkPCICompliance(gitState) {
    ...
    const pciFiles = gitState.stagedFiles.filter(
      (f) => f.includes("payment") || f.includes("card") || f.includes("pci")
    );
  ...
  async checkHIPAACompliance(gitState) {
    ...
    const hipaaFiles = gitState.stagedFiles.filter(
      (f) =>
  ...
  async checkISO27001Compliance(gitState) {
    ...
    const isoFiles = gitState.stagedFiles.filter(
      (f) => f.includes("security") || f.includes("iso") || f.includes("policy")
    );
  ...
  generateSecurityRecommendations(securityValidation, complianceChecks) {
    ...
    if (securityValidation.overallStatus === "FAIL") {
      recommendations.push(
        "🔒 Address security policy violations before proceeding"
      );
    }
  ...
  generateSecuritySummary(securityValidation, complianceChecks) {
    return {
      ...
      securityPolicies: Object.keys(securityValidation).filter(
        (k) => k !== "overallStatus" && k !== "timestamp"
      ),
      complianceStandards: Object.keys(complianceChecks).filter(
  ...
  async writeSecurityReport(report) {
    ...
    const filename = `security-policy-enforcer-${
      report.hookName
    }-${Date.now()}.json`;
