...
describe("Knowledge Hooks Stress Test", () => {
  ...
  beforeEach(async () => {
    ...
    execSync('git config user.name "Stress Test"', { cwd: testDir, stdio: "inherit" });
    execSync('git config user.email "stress@test.com"', { cwd: testDir, stdio: "inherit" });
    ...
    writeFileSync(join(testDir, "gitvan.config.js"), `
export default {
  hooks: {
    dirs: ["hooks"],
    autoEvaluate: true,
...
`);
  ...
  afterEach(() => {
    ...
    console.log(`   💾 Memory Start: ${Math.round(memoryUsage.heapUsed / 1024 / 1024)}MB`);
    console.log(`   💾 Memory End: ${Math.round(finalMemoryUsage.heapUsed / 1024 / 1024)}MB`);
    console.log(`   📈 Memory Delta: ${Math.round((finalMemoryUsage.heapUsed - memoryUsage.heapUsed) / 1024 / 1024)}MB`);
  ...
  it("should handle concurrent hook evaluations", async () => {
    ...
    for (let i = 0; i < hookCount; i++) {
      writeFileSync(join(testDir, `hooks/hook-${i}.ttl`), `
...
`);
    ...
    writeFileSync(join(testDir, "graph/init.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
...
`);
    ...
    execSync('git commit -m "Initial stress test setup"', { cwd: testDir, stdio: "inherit" });
    ...
    console.log(`   ✅ Completed ${concurrentEvaluations} concurrent evaluations`);
    console.log(`   📊 Total hooks evaluated: ${results.reduce((sum, r) => sum + r.hooksEvaluated, 0)}`);
    console.log(`   ⚡ Total hooks triggered: ${results.reduce((sum, r) => sum + r.hooksTriggered, 0)}`);
  ...
  it("should handle rapid Git operations", async () => {
    ...
    writeFileSync(join(testDir, "hooks/rapid-git-hook.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix gh: <https://gitvan.dev/graph-hook#> .
@prefix op: <https://gitvan.dev/op#> .
...
`);
    ...
    // Initial commit
    execSync("git add .", { cwd: testDir, stdio: "inherit" });
    execSync('git commit -m "Initial rapid git test"', { cwd: testDir, stdio: "inherit" });
    ...
    for (let i = 0; i < operationCount; i++) {
      operationPromises.push(
        (async () => {
          ...
          writeFileSync(join(testDir, `rapid-file-${i}.txt`), `Rapid file ${i}`);
          writeFileSync(join(testDir, `graph/file-${i}.ttl`), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
...
`);
          ...
          execSync("git add .", { cwd: testDir, stdio: "inherit" });
          execSync(`git commit -m "Rapid operation ${i}"`, { cwd: testDir, stdio: "inherit" });
  ...
  it("should handle timer-based triggers", async () => {
    ...
    writeFileSync(join(testDir, "hooks/timer-hook.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix gh: <https://gitvan.dev/graph-hook#> .
@prefix op: <https://gitvan.dev/op#> .
...
`);
    ...
    writeFileSync(join(testDir, "graph/timers.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
...
`);
    ...
    // Initial commit
    execSync("git add .", { cwd: testDir, stdio: "inherit" });
    execSync('git commit -m "Timer test setup"', { cwd: testDir, stdio: "inherit" });
    ...
    console.log(`   ⚡ Total hooks triggered: ${timerResults.reduce((sum, r) => sum + r.hooksTriggered, 0)}`);
  ...
  it("should handle memory pressure and cleanup", async () => {
    ...
    for (let i = 0; i < hookCount; i++) {
      writeFileSync(join(testDir, `hooks/memory-hook-${i}.ttl`), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix gh: <https://gitvan.dev/graph-hook#> .
@prefix op: <https://gitvan.dev/op#> .
...
`);
    ...
    for (let i = 0; i < graphCount; i++) {
      writeFileSync(join(testDir, `graph/memory-${i}.ttl`), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
...
`);
    ...
    // Initial commit
    execSync("git add .", { cwd: testDir, stdio: "inherit" });
    execSync('git commit -m "Memory test setup"', { cwd: testDir, stdio: "inherit" });
    ...
    console.log(`   📊 Mid Memory: ${Math.round(midMemory.heapUsed / 1024 / 1024)}MB`);
    console.log(`   📊 Final Memory: ${Math.round(finalMemory.heapUsed / 1024 / 1024)}MB`);
    console.log(`   📈 Memory Growth: ${Math.round((finalMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024)}MB`);
  ...
  it("should handle error conditions gracefully", async () => {
    ...
    writeFileSync(join(testDir, "hooks/error-hook-1.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix gh: <https://gitvan.dev/graph-hook#> .
@prefix op: <https://gitvan.dev/op#> .
...
`);
    ...
    writeFileSync(join(testDir, "hooks/error-hook-2.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix gh: <https://gitvan.dev/graph-hook#> .
@prefix op: <https://gitvan.dev/op#> .
...
`);
    ...
    writeFileSync(join(testDir, "graph/valid.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
...
`);
    ...
    // Initial commit
    execSync("git add .", { cwd: testDir, stdio: "inherit" });
    execSync('git commit -m "Error test setup"', { cwd: testDir, stdio: "inherit" });
    ...
    console.log(`   📊 Successful evaluations: ${errorResults.filter(r => !r.error).length}`);
    console.log(`   ❌ Failed evaluations: ${errorResults.filter(r => r.error).length}`);
  ...
  it("should handle mixed workload stress test", async () => {
    ...
    const hookTypes = [
      { type: "ResultDelta", query: "SELECT ?file ?status WHERE { ?file rdf:type gv:File . ?file gv:status ?status . }" },
      { type: "ASK", query: "ASK WHERE { ?item rdf:type gv:Item . ?item gv:active true }" },
      { type: "SELECTThreshold", query: "SELECT ?count WHERE { { SELECT (COUNT(?item) AS ?count) WHERE { ?item rdf:type gv:Item } } }" },
    ];

    hookTypes.forEach((hookType, index) => {
      writeFileSync(join(testDir, `hooks/mixed-hook-${index}.ttl`), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix gh: <https://gitvan.dev/graph-hook#> .
@prefix op: <https://gitvan.dev/op#> .
...
`);
    ...
    writeFileSync(join(testDir, "graph/mixed.ttl"), `
@prefix ex: <http://example.org/> .
@prefix gv: <https://gitvan.dev/ontology#> .
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
...
`);
    ...
    // Initial commit
    execSync("git add .", { cwd: testDir, stdio: "inherit" });
    execSync('git commit -m "Mixed workload setup"', { cwd: testDir, stdio: "inherit" });
    ...
    for (let i = 0; i < 10; i++) {
      workloadPromises.push(
        (async () => {
          writeFileSync(join(testDir, `workload-file-${i}.txt`), `Workload file ${i}`);
          execSync("git add .", { cwd: testDir, stdio: "inherit" });
          execSync(`git commit -m "Workload operation ${i}"`, { cwd: testDir, stdio: "inherit" });
    ...
    console.log(`   ⚡ Total hooks triggered: ${workloadResults.reduce((sum, r) => sum + (r.hooksTriggered || 0), 0)}`);
