...
export async function validateSecurityConfiguration(config) {
  const errors = [];
  ...
  if (config.signer) {
    if (
      ...
      !["RSA-SHA256", "RSA-SHA512"].includes(config.signer.algorithm)
    ) {
  ...
  if (config.receipts) {
    if (
      config.receipts.receiptsRef &&
      !config.receipts.receiptsRef.startsWith("refs/notes/")
    ) {
  ...
  if (config.policy) {
    ...
    const policyValidation =
      await policyValidator.validatePolicyConfiguration();
