...
describe('Pack Registry System', () => {
  ...
  describe('PackRegistry', () => {
    ...
    it('should validate registry URLs for security', () => {
      const insecureRegistry = new PackRegistry({ registryUrl: 'ftp://malicious.com' });
    ...
    it('should validate pack IDs correctly', async () => {
      ...
      // Invalid pack IDs
      const invalidIds = ['', '../malicious', 'pack with spaces', 'pack@version'];
      for (const id of invalidIds) {
        await expect(registry.get(id)).rejects.toThrow('Invalid pack ID format');
      }
    });
  ...
  describe('Marketplace', () => {
    it('should validate browse options', async () => {
      ...
      await expect(marketplace.browse(invalidOptions)).rejects.toThrow('Invalid browse options');
    });
  ...
  describe('ScaffoldRunner', () => {
    ...
    beforeEach(() => {
      ...
      writeFileSync(join(mockPackPath, 'pack.json'), JSON.stringify(manifest, null, 2));
...
`;
    ...
    it('should validate input types correctly', async () => {
      ...
      expect(() => scaffoldRunner.resolveInputs(scaffold, validInputs)).not.toThrow();

      // Invalid inputs should throw
      await expect(scaffoldRunner.resolveInputs(scaffold, invalidInputs)).rejects.toThrow();
    });
    ...
    it('should prevent path traversal attacks', () => {
      ...
      expect(() => scaffoldRunner.validateInputSecurity(dangerousInputs)).toThrow('Path traversal detected');
    });
    ...
    it('should prevent template injection', () => {
      ...
      expect(() => scaffoldRunner.validateInputSecurity(maliciousInputs)).toThrow('Template injection detected');
    });
  ...
  describe('TemplateProcessor', () => {
    ...
    it('should enforce size limits', async () => {
      ...
      await expect(processor.render(largeTemplate, {})).rejects.toThrow('Template too large');
      ...
      await expect(processor.render(largeOutputTemplate, largeData)).rejects.toThrow('Template output too large');
    });

    it('should handle all custom filters correctly', async () => {
      const testCases = [
        { filter: 'camelCase', input: 'my-test-string', expected: 'myTestString' },
        { filter: 'pascalCase', input: 'my-test-string', expected: 'MyTestString' },
        { filter: 'kebabCase', input: 'MyTestString', expected: 'my-test-string' },
        { filter: 'snakeCase', input: 'MyTestString', expected: 'my_test_string' },
        { filter: 'upperCase', input: 'hello', expected: 'HELLO' },
        { filter: 'lowerCase', input: 'HELLO', expected: 'hello' },
        { filter: 'jsEscape', input: 'say "hello"', expected: 'say \\"hello\\"' }
      ];
...
describe('{{ projectName | pascalCase }}', () => {
  it('should work', () => {
    expect(true).toBe(true);
      ...
      writeFileSync(join(packPath, 'templates', 'package.json.njk'), packageTemplate);
      mkdirSync(join(packPath, 'templates'), { recursive: true });
      writeFileSync(join(packPath, 'templates', 'test.js.njk'), testTemplate);

      // Test the full flow
      const scaffoldRunner = new ScaffoldRunner({ dryRun: false });
      ...
      try {
        const result = await scaffoldRunner.run('integration/test-pack', 'full-test', {
