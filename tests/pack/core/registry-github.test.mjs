...
describe("PackRegistry GitHub Integration", () => {
  ...
  describe("parseGitHubPackId", () => {
    ...
    it("should parse owner/repo/path format", () => {
      const result = registry.parseGitHubPackId(
    ...
    it("should parse owner/repo#ref/path format", () => {
      const result = registry.parseGitHubPackId(
        "octocat/Hello-World#develop/packages/my-pack"
      );
      expect(result).toEqual({
        owner: "octocat",
        repo: "Hello-World",
    ...
    it("should handle complex path parsing", () => {
      const result = registry.parseGitHubPackId(
        "octocat/Hello-World/packages/sub/deep"
      );
      expect(result).toEqual({
        owner: "octocat",
        repo: "Hello-World",
  ...
  describe("generateGitHubCacheKey", () => {
    it("should generate cache key for basic repo", () => {
      const key = registry.generateGitHubCacheKey(
        "octocat",
        "Hello-World",
        "main",
        null
  ...
  describe("fetchGitHubRepoMetadata", () => {
    it("should fetch repository metadata successfully", async () => {
      ...
      const result = await registry.fetchGitHubRepoMetadata(
        "octocat",
        "Hello-World"
      );
    ...
    it("should cache API responses", async () => {
      ...
      const result1 = await registry.fetchGitHubRepoMetadata(
        "octocat",
        "Hello-World"
      );
      ...
      const result2 = await registry.fetchGitHubRepoMetadata(
        "octocat",
        "Hello-World"
      );
