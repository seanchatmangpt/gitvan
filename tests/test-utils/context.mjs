...
export async function createTestContext(options = {}) {
  ...
  const prefix = options.prefix || 'gitvan-test';
  const cwd = options.cwd || join(tmpdir(), prefix, testId);
...
export async function createTestJob(cwd, jobName, options = {}) {
  const jobId = jobName; // Use jobName as the ID directly
...
export default async function run({ payload = {} } = {}) {
  ...
  // Ensure jobs directory exists and write job file
