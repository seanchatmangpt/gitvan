...
describe("PackRegistry Search & List", () => {
  ...
  describe("Search Functionality", () => {
    ...
    it("should search by tags", async () => {
      ...
      expect(results.total).toBeGreaterThan(0);
      expect(
        results.results.some((pack) => pack.tags?.includes("frontend"))
      ).toBe(true);
    });
    ...
    it("should search by capabilities", async () => {
      ...
      expect(results.total).toBeGreaterThan(0);
      expect(
        results.results.some((pack) => pack.capabilities?.includes("scaffold"))
      ).toBe(true);
    });
  ...
  describe("List Functionality", () => {
    ...
    it("should provide sort options", async () => {
      ...
      expect(results.sortOptions.some((opt) => opt.value === "name")).toBe(
        true
      );
    });
  ...
  describe("Advanced Search Features", () => {
    it("should provide search suggestions", async () => {
      ...
      expect(suggestions.some((s) => s.toLowerCase().includes("rea"))).toBe(
        true
      );
    });

    it("should parse advanced query filters", async () => {
      const results = await registry.advancedSearch(
        "tag:react author:gitvan modern"
      );

      expect(
        results.results.every(
          (pack) => pack.tags?.includes("react") && pack.author === "gitvan"
        )
      ).toBe(true);
    });
  ...
  describe("Legacy API Compatibility", () => {
    ...
    it("should include relevance scores in legacy format", async () => {
      ...
      const hasRelevance = results.some(
        (pack) => typeof pack.relevance === "number"
      );
...
async function createTestPacks(testDir) {
  ...
  writeFileSync(
    join(fullstackPackDir, "pack.json"),
    JSON.stringify(
      {
        id: "fullstack-app",
        ...
        version: "2.0.0",
        description:
          "Complete fullstack web application with React and Node.js",
