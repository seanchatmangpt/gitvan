...
async function testComprehensiveResolution() {
  ...
  try {
    ...
    console.log('   Result:', simple.map(r => r.id));
    ...
    console.log('   Result:', medium.map(r => r.id));
    ...
    const compatible = await resolver.checkCompatibility('framework/express', 'framework/fastify');
    ...
    try {
      const conflictTest = await resolver.resolveMultiple(['framework/express', 'framework/fastify']);
...
async function testGraphAnalysis() {
  ...
  try {
    ...
    console.log('   Density:', (analysis.metrics.density * 100).toFixed(1) + '%');
...
async function testComposerLogic() {
  ...
  try {
    ...
    const validation = await composer.validateComposition(['auth/jwt', 'auth/oauth']);
    ...
    const conflictValidation = await composer.validateComposition(['framework/express', 'framework/fastify']);
...
async function testEdgeCases() {
  ...
  try {
    ...
    const duplicates = await resolver.resolveMultiple(['core/base', 'core/base', 'core/utils']);
