// tests/jtbd-external-template-meaningful-work.test.mjs
// Integration tests that validate meaningful work outcomes using external templates
...
describe("JTBD External Template Meaningful Work Validation", () => {
      ...
      await withMemFSTestEnvironment(
        {
          initialFiles: {
            ...
            "templates/dashboard.njk": `# {{ quarter }} Executive Dashboard

## Key Performance Indicators

Revenue: ${{ totalRevenue | int }}
Target: ${{ targets.revenue | int }}
Achievement: {{ revenueAchievement | round(1) }}%

Units: {{ totalUnits }}
Target: {{ targets.units }}
Achievement: {{ unitsAchievement | round(1) }}%
    ...
    "units": {
      ...
      "status": "{% if unitsAchievement >= 100 %}achieved{% else %}missed{% endif %}"
        ...
        async (env) => {
          ...
          // Step 2: Generate executive dashboard using external template
          const dashboardStep = {
            id: "generate-dashboard",
            type: "template",
            config: {
              template: "file://templates/dashboard.njk",
          ...
          // Step 3: Generate actionable JSON data using external template
          const jsonStep = {
            id: "generate-json-data",
            type: "template",
            config: {
              template: "file://templates/summary.json.njk",
      ...
      await withMemFSTestEnvironment(
        {
          initialFiles: {
            ...
            "templates/types.ts.njk": `// Generated TypeScript interfaces for {{ apiSpec.service }} v{{ apiSpec.version }}
...
{% endfor %}`,
        ...
        async (env) => {
          ...
          // Step 2: Generate TypeScript interfaces using external template
          const typesStep = {
            id: "generate-types",
            type: "template",
            config: {
              template: "file://templates/types.ts.njk",
