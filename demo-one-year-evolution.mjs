...
const EVOLUTION_PHASES = [
  {
    ...
    jobs: [
      ...
      { name: "configure-environment", description: "Set up dev/staging/prod environments" },
      { name: "basic-ci", description: "Basic build, test, and deploy pipeline" }
    ]
  },
  {
    ...
    jobs: [
      ...
      { name: "documentation", description: "Auto-generated docs and changelogs" },
      { name: "branch-management", description: "Automated feature branch workflows" }
    ]
  },
  {
    ...
    jobs: [
      ...
      { name: "smart-documentation", description: "AI-generated documentation" },
      { name: "automated-refactoring", description: "AI-assisted code improvements" },
  ...
  },
  {
    ...
    jobs: [
      { name: "production-deployment", description: "Zero-downtime deployments" },
      { name: "monitoring-integration", description: "Automated health checks" },
      { name: "security-scanning", description: "Automated vulnerability detection" },
      { name: "performance-optimization", description: "Automated performance testing" }
    ]
  }
];
...
async function createEvolutionDemo() {
  ...
  console.log('This demo shows how a project evolves using GitVan over 12 months\n');
  ...
  try {
    ...
    execSync('git config user.name "GitVan Demo"', { cwd: demoDir, stdio: 'pipe' });
    execSync('git config user.email "demo@gitvan.dev"', { cwd: demoDir, stdio: 'pipe' });
    ...
    console.log('🔍 You can explore the evolution by checking git log and tags');
  }
}
...
export async function run${job.name.charAt(0).toUpperCase() + job.name.slice(1)}() {
...
async function createPhaseTemplates(demoDir, phase) {
  ...
  const templateFile = join(templatesDir, `${phase.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}.njk`);
...
async function createPhasePacks(demoDir, phase) {
  ...
  const packFile = join(packsDir, `${phase.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}-pack.json`);
  const packContent = {
...
async function showFinalSummary(git, demoDir) {
  ...
  console.log(`   Total Jobs: ${EVOLUTION_PHASES.reduce((sum, phase) => sum + phase.jobs.length, 0)}`);
