...
function testJobCommands() {
  ...
  runCommand(
    "gitvan job run --name hello",
    "Run hello job with explicit syntax"
  );
}
...
function testPackCommands() {
  ...
  runCommand(
    "gitvan pack apply builtin/nodejs-basic",
    "Apply Node.js basic pack",
    { expectFailure: true }
  );
...
function testMarketplaceCommands() {
  ...
  runCommand("gitvan marketplace browse", "Browse marketplace packs", {
    timeout: 20000,
  });
...
function testAIChatCommands() {
  ...
  runCommand(
    'gitvan chat generate "Create a simple hello world job"',
    "Generate job via AI",
    { timeout: 30000 }
  );
  ...
  runCommand(
    'gitvan chat draft "Create a React component template"',
    "Draft template via AI",
    { timeout: 30000 }
  );

  // gitvan llm call (simple prompt)
  runCommand('gitvan llm call "What is GitVan?"', "Direct AI interaction", {
    timeout: 30000,
  });
}
...
function testDaemonCommands() {
  ...
  runCommand("gitvan daemon start", "Start GitVan daemon", {
    expectFailure: true,
  });
}
...
function testAuditCommands() {
  ...
  runCommand("gitvan audit verify", "Verify operation integrity", {
    expectFailure: true,
  });
...
## Failed Commands
${RESULTS.failed
  .map((f) => `- **${f.description}**: \`${f.command}\` - ${f.error}`)
  .join("\n")}

## Successful Commands
${RESULTS.passed
  .map((p) => `- **${p.description}**: \`${p.command}\``)
  .join("\n")}

## Skipped Commands
${RESULTS.skipped
  .map((s) => `- **${s.description}**: \`${s.command}\` - ${s.reason}`)
  .join("\n")}
`;
...
async function main() {
  try {
    ...
    if (success) {
      ...
      log(
        "⚠️ Some commands failed validation. Check the report for details.",
        "error"
      );
      process.exit(1);
    }
