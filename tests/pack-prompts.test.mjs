...
describe('Pack prompting integration', () => {
  ...
  it('should resolve inputs with provided values', async () => {
    ...
    writeFileSync(join(TEST_PACK_DIR, 'pack.json'), JSON.stringify(manifest, null, 2));
    ...
    const resolved = await pack.resolveInputs(providedInputs, { noPrompt: true });
  ...
  it('should fail for missing required inputs with noPrompt', async () => {
    ...
    await expect(pack.resolveInputs({}, { noPrompt: true })).rejects.toThrow('Required input \'apiKey\' is missing');
  });
  ...
  it('should handle string to boolean conversion', async () => {
    ...
    let resolved = await pack.resolveInputs({ enabled: 'true' }, { noPrompt: true });
    expect(resolved.enabled).toBe(true);

    resolved = await pack.resolveInputs({ enabled: 'false' }, { noPrompt: true });
  ...
  it('should handle multiselect comma-separated strings', async () => {
    ...
    const resolved = await pack.resolveInputs({ plugins: 'eslint,prettier,jest' }, { noPrompt: true });
  ...
  it('should handle Pack constructor options', async () => {
    ...
    await expect(pack.resolveInputs({})).rejects.toThrow('Required input \'name\' is missing');
  });
});