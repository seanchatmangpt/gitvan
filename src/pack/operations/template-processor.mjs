  ...
  addSecurityExtensions() {
    ...
    this.env.renderString = (template, context) => {
      ...
      if (template.length > this.maxTemplateSize) {
        throw new Error(`Template too large: ${template.length} bytes (max ${this.maxTemplateSize})`);
      }
      ...
      if (result.length > this.maxOutputSize) {
        throw new Error(`Template output too large: ${result.length} bytes (max ${this.maxOutputSize})`);
      }
  ...
  async process(step, context = {}) {
    ...
    try {
      ...
      if (!this.isTemplate(template)) {
        ...
        return this.copyFile(src, target, backup);
      }
