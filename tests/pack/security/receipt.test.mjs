...
describe("ReceiptManager", () => {
  ...
  describe("create", () => {
    it("should create a receipt with all required fields", async () => {
      const receipt = await receipts.create(pack, "install", "success", {
        duration: 1500,
      });
  ...
  describe("write and read", () => {
    ...
    it("should read all receipts when no packId specified", async () => {
      ...
      const otherReceipt = await receipts.create(
        otherPack,
        "uninstall",
        "success"
      );
    ...
    it("should return latest receipt when requested", async () => {
      ...
      expect(new Date(latest.timestamp).getTime()).toBeGreaterThan(
  ...
  describe("verify", () => {
    ...
    it("should verify commit exists", async () => {
      ...
      expect(result.errors).toContain(
        "Commit not found: nonexistent-commit-hash"
      );
    });
  });
  ...
  describe("listByOperation", () => {
    beforeEach(async () => {
      ...
      const anotherInstallReceipt = await receipts.create(
        pack,
        "install",
        "failure"
      );
  ...
  describe("exportReceipts", () => {
    ...
    it("should reject unsupported export formats", async () => {
      await expect(receipts.exportReceipts("xml")).rejects.toThrow(
