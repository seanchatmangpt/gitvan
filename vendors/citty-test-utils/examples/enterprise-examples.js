...
export async function basicDomainCommands() {
  ...
  const serverResult = await runLocalCitty([
    'infra',
    'server',
    'create',
    '--type',
  ...
  const projectResult = await runLocalCitty([
    'dev',
    'project',
    'create',
    '--name',
  ...
  const userResult = await runLocalCitty([
    'security',
    'user',
    'create',
    '--name',
...
export async function fluentCommandBuilder() {
  ...
  serverResult.expectSuccess().expectResourceCreated('infra', 'server', 'web-server-002')
  ...
  projectResult.expectSuccess().expectResourceCreated('dev', 'project', 'enterprise-app')
  ...
  networkResult.expectSuccess().expectResourceCreated('infra', 'network', 'network-001')
