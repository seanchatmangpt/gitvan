...
export async function setupCleanroom({
  ...
  if (!singleton) {
    ...
    try {
      ...
      throw new Error(`Failed to setup cleanroom: ${error.message}. Make sure Docker is running and accessible.`);
    }
  }
  return singleton;
}

export async function runCitty(args, { json = false, cwd = "/app", timeout = 30000 } = {}) {
  if (!singleton)
